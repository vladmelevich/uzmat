# 🐳 Инструкция по деплою на VPS сервер

## 📋 Требования

- VPS сервер с Ubuntu/Debian
- Docker и Docker Compose установлены
- IP сервера: `109.199.127.149`

## 🚀 Быстрый старт

### 1. Подготовка на сервере

```bash
# Установите Docker (если еще не установлен)
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# Установите Docker Compose
sudo curl -L "https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
sudo chmod +x /usr/local/bin/docker-compose

# Создайте директорию для проекта
mkdir -p /var/www/uzmat
cd /var/www/uzmat
```

### 2. Загрузка проекта

```bash
# Клонируйте репозиторий или загрузите файлы
git clone <your-repo-url> .
# ИЛИ загрузите файлы через scp/sftp
```

### 3. Настройка переменных окружения

```bash
# Скопируйте пример файла
cp env.example .env

# Отредактируйте .env файл
nano .env
```

**Важно заполнить в .env:**
- `SECRET_KEY` - сгенерируйте новый ключ
- `DEBUG=False` - для production
- `ALLOWED_HOSTS` - добавьте IP сервера
- `DB_PASSWORD` - надежный пароль для БД
- `MYSQL_ROOT_PASSWORD` - надежный пароль для root MySQL

### 4. Первый запуск

```bash
# Сделайте скрипты исполняемыми
chmod +x deploy.sh backup.sh

# Запустите деплой
./deploy.sh
```

## 🔄 Обновление приложения

### Автоматическое обновление (рекомендуется)

```bash
./deploy.sh
```

Скрипт автоматически:
1. Создаст бэкап БД
2. Остановит контейнеры
3. Обновит код
4. Пересоберет образы
5. Применит миграции
6. Запустит контейнеры

### Ручное обновление

```bash
# 1. Бэкап БД
./backup.sh

# 2. Обновите код
git pull  # или загрузите новые файлы

# 3. Пересоберите и перезапустите
docker-compose build
docker-compose up -d

# 4. Примените миграции
docker-compose exec web python manage.py migrate

# 5. Соберите статику
docker-compose exec web python manage.py collectstatic --noinput
```

## 📦 Бэкапы

### Создание бэкапа вручную

```bash
./backup.sh
```

Бэкапы сохраняются в папку `backups/` с автоматическим сжатием.

### Восстановление из бэкапа

```bash
# Распакуйте бэкап
gunzip backups/backup_YYYYMMDD_HHMMSS.sql.gz

# Восстановите
docker exec -i uzmat_mysql mysql -u root -p${MYSQL_ROOT_PASSWORD} ${DB_NAME} < backups/backup_YYYYMMDD_HHMMSS.sql
```

## 🔧 Полезные команды

### Просмотр логов

```bash
# Все контейнеры
docker-compose logs -f

# Только Django
docker-compose logs -f web

# Только MySQL
docker-compose logs -f db

# Только Nginx
docker-compose logs -f nginx
```

### Вход в контейнеры

```bash
# Django контейнер
docker-compose exec web bash

# MySQL контейнер
docker-compose exec db bash

# Выполнить команду Django
docker-compose exec web python manage.py shell
docker-compose exec web python manage.py createsuperuser
```

### Остановка/Запуск

```bash
# Остановить все
docker-compose down

# Запустить все
docker-compose up -d

# Перезапустить
docker-compose restart
```

### Просмотр статуса

```bash
docker-compose ps
docker stats
```

## 🗄️ Структура данных

### Volumes (данные сохраняются)

- `mysql_data` - база данных MySQL (все данные пользователей)
- `./media` - загруженные файлы (фото, аватары)
- `./staticfiles` - собранные статические файлы
- `./backups` - бэкапы базы данных

**Важно:** Все эти данные хранятся на хосте и не удаляются при обновлении контейнеров!

## 🔒 Безопасность

### 1. Настройте файрвол

```bash
# Разрешите только нужные порты
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp
sudo ufw allow 22/tcp  # SSH
sudo ufw enable
```

### 2. Настройте SSL (Let's Encrypt)

```bash
# Установите certbot
sudo apt install certbot python3-certbot-nginx

# Получите сертификат
sudo certbot --nginx -d your-domain.com

# Автоматическое обновление
sudo certbot renew --dry-run
```

После получения сертификата:
1. Обновите `nginx.conf` (раскомментируйте HTTPS секцию)
2. Добавьте домен в `ALLOWED_HOSTS` и `CSRF_TRUSTED_ORIGINS` в `.env`
3. Перезапустите: `docker-compose restart nginx`

### 3. Регулярные бэкапы

Настройте cron для автоматических бэкапов:

```bash
# Откройте crontab
crontab -e

# Добавьте строку (бэкап каждый день в 3:00)
0 3 * * * cd /var/www/uzmat && ./backup.sh
```

## 🐛 Решение проблем

### Контейнер не запускается

```bash
# Проверьте логи
docker-compose logs web

# Проверьте переменные окружения
docker-compose exec web env | grep DB_
```

### Ошибки подключения к БД

```bash
# Проверьте, запущен ли MySQL
docker-compose ps db

# Проверьте логи MySQL
docker-compose logs db

# Проверьте подключение
docker-compose exec web python manage.py dbshell
```

### Проблемы с миграциями

```bash
# Покажите статус миграций
docker-compose exec web python manage.py showmigrations

# Откатите последнюю миграцию
docker-compose exec web python manage.py migrate app_name migration_name

# Создайте новую миграцию
docker-compose exec web python manage.py makemigrations
```

### Очистка (если нужно начать заново)

```bash
# ⚠️ ВНИМАНИЕ: Это удалит все данные!
docker-compose down -v
docker system prune -a
```

## 📞 Доступ к сайту

После успешного деплоя сайт будет доступен:

- HTTP: `http://109.199.127.149`
- После настройки SSL: `https://109.199.127.149`

## 📝 Чеклист перед production

- [ ] Изменен `SECRET_KEY` на уникальный
- [ ] `DEBUG=False` в `.env`
- [ ] Настроены надежные пароли для БД
- [ ] Добавлен IP/домен в `ALLOWED_HOSTS`
- [ ] Настроен файрвол
- [ ] Настроен SSL сертификат
- [ ] Настроены автоматические бэкапы
- [ ] Протестированы миграции
- [ ] Создан суперпользователь Django

## 🎯 Мониторинг

### Проверка здоровья контейнеров

```bash
docker-compose ps
```

Все контейнеры должны быть в статусе `Up`.

### Проверка использования ресурсов

```bash
docker stats
```

## 📚 Дополнительные ресурсы

- [Docker документация](https://docs.docker.com/)
- [Docker Compose документация](https://docs.docker.com/compose/)
- [Django deployment checklist](https://docs.djangoproject.com/en/stable/howto/deployment/checklist/)

